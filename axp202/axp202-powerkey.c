...
static int axp20_powerkey_probe(struct platform_device *pdev)
{
	...
	if (pdev->dev.of_node) {
		...
		ret = axp_powerkey_dt_parse(pdev->dev.of_node, &axp20_config);
		if (ret) {
			pr_err("%s parse device tree err\n", __func__);
			return -EINVAL;
		}
	} else {
		pr_err("axp20 powerkey device tree err!\n");
		return -EBUSY;
	}
	...
	axp_regmap_read(map, AXP20_OFF_CTL, &val);
	val &= 0xfc;
	val |= (axp20_config.pmu_pwrnoe_time) / 1000;
	axp_regmap_write(map, AXP20_OFF_CTL, val);

	/* pek open time set */
	axp_regmap_read(map, AXP20_POK_SET, &val);
	...
	axp_regmap_write(map, AXP20_POK_SET, val);
	...
	if (axp20_config.pmu_powkey_off_en)
		...
		axp20_config.pmu_powkey_off_en = 0;
	...
	axp_regmap_read(map, AXP20_POK_SET, &val);
	val &= 0xfc;
	val |= (axp20_config.pmu_powkey_off_time - 4000) / 2000;
	axp_regmap_write(map, AXP20_POK_SET, val);
...
static int axp20_powerkey_remove(struct platform_device *pdev)
{
	int i, irq;
...
static const struct of_device_id axp20_powerkey_dt_ids[] = {
	{ .compatible = "axp209-powerkey", },
	{},
};
MODULE_DEVICE_TABLE(of, axp20_powerkey_dt_ids);

static struct platform_driver axp20_powerkey_driver = {
	.driver = {
		.name = "axp20-powerkey",
		.of_match_table = axp20_powerkey_dt_ids,
	},
	.probe  = axp20_powerkey_probe,
	.remove = axp20_powerkey_remove,
};

module_platform_driver(axp20_powerkey_driver);

MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("onkey Driver for axp20x PMIC");
